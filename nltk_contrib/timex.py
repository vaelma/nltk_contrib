...
dmy = r"(year|day|week|month)"
rel_day = r"(today|yesterday|tomorrow|tonight|tonite)"
exp1 = r"(before|after|earlier|later|ago)"
exp2 = r"(this|next|last)"
iso = r"\d+[/-]\d+[/-]\d+ \d+:\d+:\d+\.\d+"
year = r"((?<=\s)\d{4}|^\d{4})"
regxp1 = r"((\d+|(" + numbers + r"[-\s]?)+) " + dmy + r"s? " + exp1 + r")"
